...
For a merged identity the block is demanded on the same declaration a citation would sit on, which is the declaration the graph names.
...
→ [Full-Stack Wiring](/docs/evidence/wiring) puts all four rules on a real monorepo.
