...
| `symbol` | all | Which declaration kinds may host an `@evidence` tag. |
...
A citation covers its target and every selected descendant. So `symbol: "property"` on a reference can still be satisfied by one `@evidence IShoppingSale ...`, and one `@evidence prisma:Sale ...` discharges every selected column and relation beneath that model.
...
The constraints belong to one reference and never pool. An exclusion the Swagger reference above refuses may still satisfy a Markdown reference in the same claim, and two references over the same files stay independent.
