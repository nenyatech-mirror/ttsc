...
Rendered prose stays clean, so the document a reader sees is unchanged. A file-level citation sits at the top of the document; a heading-level citation sits right below its heading.
...
- Markdown and Swagger claim hosts keep their selected-symbol behavior.
...
→ [Rules](/docs/evidence/rules) for the four rules that read these tags, or [Full-Stack Wiring](/docs/evidence/wiring) for a graph that uses every target form.
