...
Then cache `.ttsc-cache/plugins` as the required artifact. Add `.ttsc-cache/go-build` only as the optional accelerator. Point the root at a directory dedicated to `ttsc`: `ttsc clean` removes its `plugins/` and `go-build/` there, so a shared location such as `~/.cache` is the wrong target.

Relocating costs you automatic cleanup. In the default project-local cache, `ttsc` prunes plugin binaries unused for 30 days with a 2 GiB LRU ceiling. It performs an idle Go-object scan at most daily and enforces the same policy after every cold source-plugin build attempt, including a failed build, pruning from an 8 GiB ceiling toward 6 GiB. The newest target-sized cohort remains protected for the requested hour plus Go's one-hour access-mtime coalescing tolerance. Active builds and maintenance publish heartbeat records. A stopped build heartbeat retains a one-hour grace for an orphaned Go child; completed maintenance is retired immediately, and an abandoned maintenance heartbeat expires after one minute. Records dated implausibly far in the future are discarded instead of blocking builders through clock skew or restored metadata. A root named explicitly through `TTSC_CACHE_DIR` or `--cache-dir` is never garbage collected, on the assumption that a caller who named a path owns it. `TTSC_GO_CACHE_DIR` follows the same caller-owned lifetime, and an ambient user `GOCACHE` is never modified or removed by ttsc.
