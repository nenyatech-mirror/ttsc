...
Relocating costs you automatic cleanup. In the default project-local cache, `ttsc` prunes plugin binaries unused for 30 days with a 2 GiB LRU ceiling. It also scans the ttsc-owned Go object cache at most daily, pruning oldest objects from an 8 GiB ceiling toward 6 GiB while reserving the newest target-sized set used in the last hour. Active ttsc Go builds publish leases, so maintenance yields instead of deleting an object during compilation. A root named explicitly through `TTSC_CACHE_DIR` or `--cache-dir` is never garbage collected, on the assumption that a caller who named a path owns it. `TTSC_GO_CACHE_DIR` follows the same caller-owned lifetime, and an ambient user `GOCACHE` is never modified or removed by ttsc.
