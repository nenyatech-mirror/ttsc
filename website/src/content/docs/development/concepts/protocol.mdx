...
Each variable is set per spawn and describes only the current invocation. A value inherited from an ancestor `ttsc` process is cleared rather than passed on, so a nested build never sees the outer run's flags.
