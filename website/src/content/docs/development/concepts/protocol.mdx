...
- `hostInputHashes`: optional evaluation-time fingerprints for `hostInputs`. Use a lowercase SHA-256 digest for an observed file and `null` for a missing candidate. Capture the fingerprint when the descriptor reads or probes the input, before module/config resolution can select a result. Ttsc still watches an input without a fingerprint, but persistent bundler adapters decline narrow cache reuse because they cannot prove which state produced the descriptor. A linked native plugin reports the same evidence with `PluginContext.ReportHostInputHash(absolutePath, &digest)` or a nil digest for a missing candidate; contradictory observations keep the path in `hostInputs` and omit its fingerprint.
