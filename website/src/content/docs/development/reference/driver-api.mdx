...
A linked plugin that reads or probes configuration calls `ctx.ReportHostInputHash(path, &digest)` at the observation point, using a lowercase SHA-256 digest for a file and nil for a missing candidate. The path is also added to the transformation's `hostInputs`. If the plugin can identify an input but cannot fingerprint it at evaluation time, call `ctx.ReportHostInput(path)`; persistent adapters keep watching it but decline narrow generation reuse. Reporting two different states for one path omits its hash and preserves the path, so a concurrent write cannot be certified as stable.
