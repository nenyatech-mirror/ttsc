...
Enforce trailing semicolons on statements that need them, and on the members that need them: an interface, type-literal, or class member carrying no body is terminated once the list it sits in is broken across lines. A member with a braced body is never followed by one, and a comma-separated list (an object literal's accessors) is left alone. Default: required. Autofixable. Driven by `format.semi`.
