...
A file already formatted by Prettier is therefore left byte-identical, and a file that is not is normalized on every axis the key table below lists and on no other.
