...
[`@ttsc/evidence`](/docs/lint/evidence) is a first-party contributor built exactly this way: a descriptor whose `source` points at its own Go directory, a `peerDependencies` entry on `@ttsc/lint`, and four rules a consumer registers from `lint.config.ts`. Read it when a paragraph above needs a real package to point at.
