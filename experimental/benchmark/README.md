...
| `website/public/benchmark/graph.json` | Graph dashboard data. `src/executable/graph/index.ts` upserts only measured cells by harness, tool, repo, prompt id or family, stable model tier, effort, fixture branch, and daemon mode. For parallel `--no-website` graph runs, publish afterward with `pnpm --dir experimental/benchmark graph:publish -- --from <out-dir>`. `src/executable/graph/index-time.ts` owns only the top-level `index` key (`{ host, scale, cells }`), upserted by (project, tool). |
