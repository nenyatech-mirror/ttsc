...
```bash
node --experimental-strip-types src/performance.ts                          # full sweep
node --experimental-strip-types src/performance.ts --project=vue            # one fixture
node --experimental-strip-types src/performance.ts --setup-only             # clone + install, no measurement
node --experimental-strip-types src/performance.ts --list                   # print the cell grid and exit
node --experimental-strip-types src/performance.ts --verbose                # tee child stdio for debugging
node --experimental-strip-types src/graph.ts --project=typeorm --models=gpt-5.4-mini --tools=ttsc-graph,codegraph,codebase-memory,serena # one graph AI-token benchmark
node --experimental-strip-types src/graph.ts --all --models=gpt-5.4-mini --arm=baseline --tools=baseline --prompt-family=all --runs=5 --reset # baseline-only graph refresh
node --experimental-strip-types src/graph.ts --all --models=gpt-5.4-mini --arm=graph --tools=ttsc-graph,codegraph,codebase-memory,serena --prompt-family=all --runs=5 # comparator graph sweep
node --experimental-strip-types src/graph/audit-codex-traces.ts --dir=.work/graph/<timestamp> # inspect Codex message/tool/reasoning ledger and baseline savings
node --experimental-strip-types src/graph/audit-codex-traces.ts --self-test # verify audit parser and savings semantics
node --experimental-strip-types src/graph/bench.ts --project=../../packages/ttsc --runs=5 # structural graph metrics
node --experimental-strip-types src/graph/index-time.ts --all # cold index build time per (tool x fixture), quiet host only
```

## Layout

- `src/`: TypeScript benchmark runners and their shared implementation.
- `src/graph/`: Graph benchmark harnesses, trace tools, and publication code.
...
Per-project commands, install/prepare overrides, and prerequisites live in `PACKAGE_CONFIGS` at the top of `src/performance.ts`.
...
| Flag | Effect |
| --- | --- |
| `--models gpt-5.4-mini` | Select agent models for `graph.ts`. `codex` resolves to `--codex-model` and always uses effort `high`. |
| `--tools ttsc-graph,codegraph,codebase-memory,serena` | Select graph tools for `graph.ts`. Use `all` for every graph tool, or `baseline` with `--arm=baseline` to record only the empty-MCP baseline. |
...
| `--prompt-family dedicated,common` | Select manifest prompt families for `graph.ts`. `all` expands to both. |
| `--branch ttsc` / `--fixture-branch ttsc` | Select the fixture branch for `graph.ts`; allowed values are `ttsc` and `ttsc-lint`. |
...
| --- | --- | --- |
| `TTSC_BENCH_WORK` | `./.work` (`performance.ts`), `../../../ttsc-benchmark-work` (`graph.ts`) | Clone working directory. The graph runner defaults outside the repo so a measured agent does not inherit ttsc's `CLAUDE.md` / `AGENTS.md` from a parent directory. |
...
| `website/public/benchmark/graph.json` | Graph dashboard data. `graph.ts` upserts only measured cells by harness, tool, repo, prompt id or family, stable model tier, effort, fixture branch, and daemon mode. For parallel `--no-website` graph runs, publish afterward with `node --experimental-strip-types experimental/benchmark/src/graph/publish.ts --from <out-dir>`. `graph/index-time.ts` owns only the top-level `index` key (`{ host, scale, cells }`), upserted by (project, tool). |
