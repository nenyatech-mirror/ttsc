...
| Flag | Effect |
| --- | --- |
| `--models gpt-5.4-mini` | Select agent models for `src/executable/graph/index.ts`. `codex` resolves to `--codex-model` and always uses effort `high`. |
| `--tools ttsc-graph,codegraph,codebase-memory,serena` | Select graph tools for `src/executable/graph/index.ts`. Use `all` for every graph tool, or `baseline` with `--arm=baseline` to record only the empty-MCP baseline. |
...
| `--prompt-family dedicated,common` | Select manifest prompt families for `src/executable/graph/index.ts`. `all` expands to both. |
...
| `--tools ttsc-graph,codegraph,codebase-memory,serena` (index-time) | Select tools for `src/executable/graph/index-time.ts`; defaults to `all`. |
| `--no-setup` / `--no-install` (index-time) | Skip fixture cloning / dependency installation in `src/executable/graph/index-time.ts`. |
...
| --- | --- | --- |
| `TTSC_BENCH_WORK` / `TTSC_GRAPH_BENCH_WORK` | `./.work` (`src/executable/performance/index.ts`), `../graph-benchmark-work` (`src/executable/graph/index.ts`) | Clone working directory. The graph runner defaults outside the repo so a measured agent does not inherit ttsc's `CLAUDE.md` / `AGENTS.md` from a parent directory. |
...
| `website/public/benchmark/graph.json` | Graph dashboard data. `src/executable/graph/index.ts` upserts measured cells by the visible axes: harness, tool, repo, prompt id or family, stable model tier, and daemon mode. Fixture branch, effort, and setup time remain metadata and never fork a visible cell. For parallel `--no-website` graph runs, publish afterward with `pnpm --dir experimental/benchmark graph:publish -- --from <out-dir>`. `src/executable/graph/index-time.ts` owns only the top-level `index` key (`{ host, scale, cells }`), upserted by (project, tool). |
