...
| --- | --- | --- |
| `TTSC_BENCH_WORK` / `TTSC_GRAPH_BENCH_WORK` | `./.work` (`performance.ts`), `../graph-benchmark-work` (`graph.ts`) | Clone working directory. The graph runner defaults outside the repo so a measured agent does not inherit ttsc's `CLAUDE.md` / `AGENTS.md` from a parent directory. |
