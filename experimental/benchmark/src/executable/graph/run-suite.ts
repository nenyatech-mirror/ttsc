...
const tmpDir = path.join(TtscBenchmarkConstant.WORK_ROOT, "graph", "suite-tmp");
...
function runPrompt(prompt: ITtscBenchmarkGraphPrompt): Promise<IPromptResult> {
...
function websiteCellFromReport(data: unknown): IStoredWebsiteAgentCell | null {
...
function isGraphBenchmarkPrompt(
  ...
  return (
    ...
    (prompt.fixtureBranch === undefined || prompt.fixtureBranch === "graph") &&
