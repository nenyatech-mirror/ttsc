...
import type { ITtscBenchmarkGraphPrompt } from "../../graph/structures/ITtscBenchmarkGraphPrompt.ts";
import type { ITtscBenchmarkGraphRepository } from "../../graph/structures/ITtscBenchmarkGraphRepository.ts";
import type { TtscBenchmarkProcess } from "../../graph/structures/TtscBenchmarkProcess.ts";
...
/**
 * Loads the reusable graph benchmark prompts from `questions/manifest.json`.
 *
 * The manifest is external JSON rather than trusted TypeScript. Validate every
 * field used to select a repository or read a question file so malformed input
 * stops the benchmark before it can run a different workload.
 */
function loadManifest(): ITtscBenchmarkGraphPrompt.IManifest {
  ...
  if (!isGraphBenchmarkPromptManifest(parsed)) {
    throw new Error(`invalid graph benchmark prompt manifest: ${manifestPath}`);
  }
  return parsed;
}
 ...
 *
 * Schema version one is the only format this runner understands. Every prompt
 * field that affects fixture selection, question loading, or report provenance
 * is checked before the value enters the typed benchmark pipeline.
 */
...
function isGraphBenchmarkPrompt(
  ...
  return (
    ...
    (value.fixtureBranch === undefined || value.fixtureBranch === "graph") &&
 ...
 * in-root symlink that targets an external file must not change the measured
 * question. The manifest digest is then compared with the raw file bytes before
 * the text is trimmed for the agent prompt.
 */
function resolveManifestPrompt(
  ...
  if (!isContainedPath(TtscBenchmarkConstant.QUESTIONS_ROOT, questionFile)) {
    throw new Error(
...
function spawnAsync(
  command: string,
  commandArgs: string[],
  { input, inputDelayMs = 0, ...spawnOpts }: TtscBenchmarkProcess.ISpawnOptions,
 ...
 *
 * A value beginning with `[` is an explicit JSON representation and must be a
 * string array. Other values retain the benchmark's shell-like convenience
 * syntax, whose tokenizer already produces strings without coercing JSON data.
 */
function parseConfiguredArgs(raw: string, targetRepoDir: string): string[] {
  ...
  return parsed.map((part) =>
    part.replaceAll("{repo}", targetRepoDir).replaceAll("{cwd}", targetRepoDir),
  );
}
...
/** Tests whether an unknown content item has the stream block fields we read. */
...
/** Tests whether an unknown assistant usage object contains numeric counters. */
function isClaudeStreamUsage(
  value: unknown,
): value is NonNullable<NonNullable<IClaudeStreamEvent["message"]>["usage"]> {
  return (
    isRecord(value) &&
...
/** Tests whether an unknown assistant message contains validated stream blocks. */
...
/** Tests whether an unknown model-usage map is safe to rank by output tokens. */
...
/**
 * Validates every Claude stream event field consumed by benchmark accounting.
 *
 * Unknown event kinds remain accepted when their common fields are well typed;
 * the parser intentionally ignores those kinds while retaining compatibility
 * with newer Claude CLI streams.
 */
