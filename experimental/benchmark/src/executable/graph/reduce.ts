#!/usr/bin/env node
/**
 * Reduces a raw graph dump into the website viewer payload.
 *
 * The executable owns filesystem and Go orchestration. The reusable pure
 * transform remains in `TtscBenchmarkGraphReduce`.
 */
...
import { TtscBenchmarkGraphReduce } from "../../graph/TtscBenchmarkGraphReduce.ts";
...
const parsed: TtscBenchmarkCommandLine.IArguments =
  TtscBenchmarkCommandLine.parse(process.argv.slice(2), {
    values: ["in", "max-nodes", "name", "out", "project", "root", "tsconfig"],
  });
...
function selectDump(): { name: string; raw: IRawDump } {
  if (parsed.flags.has("--demo"))
    return {
      name: parsed.values.name ?? "sample",
      raw: createDemoDump(),
    };
  if (parsed.values.in !== undefined) {
    const raw: IRawDump = parseRawDump(
      fs.readFileSync(parsed.values.in, "utf8"),
    );
    return {
      name: parsed.values.name ?? raw.project ?? "graph",
      raw,
    };
  }
  if (parsed.values.project !== undefined) {
    const root: string | undefined = parsed.values.root;
    const tsconfig: string | undefined = parsed.values.tsconfig;
    if (root === undefined || tsconfig === undefined)
      throw new Error("--project needs --root <fixtureDir> --tsconfig <path>");
    return {
      name: parsed.values.name ?? parsed.values.project,
      raw: dumpFromGo(root, tsconfig),
    };
  }
  throw new Error(
...
function isRawDump(input: unknown): input is IRawDump {
  return (
    ...
    (input.provenance === undefined || typeof input.provenance === "string") &&
