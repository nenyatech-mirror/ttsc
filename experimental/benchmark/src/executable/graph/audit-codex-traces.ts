...
function buildAudit({
  ...
  if (suiteDir === null && suiteReport === null) {
    throw new Error("buildAudit requires a suite directory or suite report");
  }
  const rootDir =
    suiteDir ?? path.dirname(requireString(suiteReport, "suite report path"));
...
function analyzeGraphPayload(
  ...
  if (kind === "details") {
    ...
    const memberLists = nodes.map((node) => objectArray(node.members) ?? []);
...
function isGraphReplaceableCommand(command: string, output: string): boolean {
...
function isStoredAudit(value: unknown): value is StoredAudit {
  if (
    ...
    !(
      value.baselineSource === null || typeof value.baselineSource === "string"
    ) ||
...
function runSelfTest(): void {
  ...
  assertSelf(
    comparisonDelta.deltaFromFirst.totalLowerBoundTokensSavedVsBaseline === 12,
  ...
  assertSelf(
    syntheticRun.usage.ledger.inputTokensNotExplainedByVisibleTraceMaterial >=
      0,
  ...
  assertSelf(
    number(suite.topMcpHotspots[0]?.estimatedExactAvoidableOutputTokens) > 0,
    "MCP hotspot should report duplicate exact output",
  );
  assertSelf(
    number(suite.topMcpHotspots[0]?.estimatedExactAvoidablePromptReplayTokens) >
      0,
