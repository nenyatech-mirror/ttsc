 ...
 * structurally invalid manifest must stop the benchmark before it can select a
 * wrong fixture, question, or provenance hash.
 */
...
/** Tests whether an unknown manifest entry contains one complete graph prompt. */
function isGraphBenchmarkPrompt(
  ...
  return (
    ...
    (input.fixtureBranch === undefined || input.fixtureBranch === "graph") &&
 ...
 * `TtscBenchmarkConstant.QUESTIONS_ROOT`, including after symlink resolution.
 * Its raw file contents must match the manifest hash before the trimmed
 * question enters a measured agent run.
 */
...
/** Tests whether a resolved target is a strict descendant of a resolved root. */
...
type ICodexStreamEvent = ICodexItemCompletedEvent | ICodexTurnCompletedEvent;
 ...
 *
 * Unrelated event kinds are ignored. Measured kinds fail closed unless their
 * nested payloads have the types required by the token and tool-call metrics.
 */
...
/** Tests whether a completed Codex item has the fields needed by its item kind. */
function isCodexStreamItem(input: unknown): input is ICodexStreamItem {
  if (!TtscBenchmarkObject.isRecord(input) || typeof input.type !== "string")
    return false;
  if (input.command !== undefined && typeof input.command !== "string") {
    return false;
  }
  if (input.text !== undefined && typeof input.text !== "string") return false;
  if (input.type === "command_execution" && typeof input.command !== "string") {
    return false;
  }
 ...
 * untrusted CLI output cannot be coerced into a successful benchmark sample.
 */
function parseStream(text: string, durMs: number): ITtscBenchmarkAgentSample {
  let tokens = 0,
    cached = 0,
    reasoning = 0,
    turns = 0,
