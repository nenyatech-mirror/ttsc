...
interface IAgentHarnessReport {
  ...
  repo?: string;
  runs?: number;
  samples: Record<ITtscBenchmarkAgentSample.Arm, ITtscBenchmarkAgentSample[]>;
}
...
function sanitizeSample(sample: ITtscBenchmarkAgentSample): PublishedSample {
...
function ensureCodegraphIndex(project: string, repoDir: string): number | null {
 ...
 */
function ensureSerenaIndex(project: string, repoDir: string): number | null {
...
function isAgentHarnessReport(value: unknown): value is IAgentHarnessReport {
...
function isAgentSamples(
  value: unknown,
): value is Record<ITtscBenchmarkAgentSample.Arm, ITtscBenchmarkAgentSample[]> {
  return (
    isRecord(value) &&
