...
- **Scenario D: the same envelope without a build boundary** (the Vite development server's persistent-validation mode). Each module owns one disjoint external input. The gate requires reads per module to stay bounded by that file's inputs instead of growing with the whole envelope union.
