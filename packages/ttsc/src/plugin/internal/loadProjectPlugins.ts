...
export const COMMONJS_PLUGIN_DESCRIPTOR_SHIM_SOURCE = [
  ...
  `            const failedFile = asFile(url);`,
  `            moduleLoadFailures.set(error, { retryWithTtsx: error.code === "ERR_UNKNOWN_FILE_EXTENSION" && typeof failedFile === "string" && /\\.(?:[cm]?ts|tsx)$/i.test(failedFile) });`,
  `          }`,
  `          throw error;`,
  `        }`,
  `      },`,
